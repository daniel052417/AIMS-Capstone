...
export interface UpdateStaffUserData extends Partial<CreateStaffUserData> {
  id: string;
}
...
export const userApi = {
  // Get all users
  async getAllUsers(): Promise<StaffUser[]> {
    const { data, error } = await supabase
      .from('users')
      .select('*')
  ...
  // Create new user
  async createUser(userData: CreateStaffUserData): Promise<StaffUser> {
  ...
  // Update user
  async updateUser(userData: UpdateStaffUserData): Promise<StaffUser> {
...
export const staffUserApi = {
  ...
  async createUserForStaff(staffId: string, userData: CreateStaffUserData): Promise<{ user: StaffUser; link: StaffUserLink }> {
...
export const enhancedStaffApi = {
  ...
  async createStaffWithAccount(
  ...
  ): Promise<{ staff: Staff; user?: StaffUser; workflow?: AccountCreationWorkflow }> {
    ...
    if (createUserAccount && accountDetails) {
      ...
      if (accountDetails.sendEmailInvite) {
        ...
        const userData: CreateStaffUserData = {
          email: staff.email,
