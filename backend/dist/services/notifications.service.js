...
class NotificationsService {
    ...
    static async getNotificationTemplateById(id) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('notification_templates')
                .select('*')
    ...
    static async getClientNotifications(filters = {}) {
        try {
            let query = supabaseClient_1.supabaseAdmin
    ...
    static async createClientNotification(notificationData) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('client_notifications')
                .insert([notificationData])
    ...
    static async updateClientNotification(id, notificationData) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('client_notifications')
    ...
    static async deleteClientNotification(id) {
        try {
            const { error } = await supabaseClient_1.supabaseAdmin
                .from('client_notifications')
                .delete()
    ...
    static async sendClientNotification(id) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('client_notifications')
                .update({
    ...
    static async sendSystemNotification(notificationData) {
        try {
            let userQuery = supabaseClient_1.supabaseAdmin
                .from('users')
                .select('id')
                .eq('is_active', true);
    ...
    static async getNotificationStats(userId) {
        try {
            const [totalNotifications, unreadNotifications, notificationsByType] = await Promise.all([
                supabaseClient_1.supabaseAdmin.from('notifications').select('id', { count: 'exact' }).eq('user_id', userId),
                supabaseClient_1.supabaseAdmin.from('notifications').select('id', { count: 'exact' }).eq('user_id', userId).eq('is_read', false),
                supabaseClient_1.supabaseAdmin.from('notifications').select('type').eq('user_id', userId)
            ]);
