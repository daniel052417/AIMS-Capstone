...
exports.InventoryClerkService = void 0;
const supabaseClient_1 = require("../../config/supabaseClient");
class InventoryClerkService {
    static async getDashboard() {
        try {
            const [productsResult, categoriesResult, suppliersResult, lowStockResult, transactionsResult] = await Promise.all([
                supabaseClient_1.supabaseAdmin.from('products').select('id', { count: 'exact' }),
                supabaseClient_1.supabaseAdmin.from('categories').select('id', { count: 'exact' }),
                supabaseClient_1.supabaseAdmin.from('suppliers').select('id', { count: 'exact' }),
                supabaseClient_1.supabaseAdmin.from('products').select('id', { count: 'exact' }).lte('stock_quantity', 'minimum_stock'),
                supabaseClient_1.supabaseAdmin.from('inventory_transactions').select('id', { count: 'exact' })
            ]);
            return {
    ...
    static async getInventoryTransactions(filters = {}) {
        try {
            let query = supabaseClient_1.supabaseAdmin
    ...
    static async createInventoryTransaction(transactionData) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('inventory_transactions')
    ...
    static async getStockAdjustments(filters = {}) {
        try {
            let query = supabaseClient_1.supabaseAdmin
    ...
    static async getPurchaseOrderItems(orderId) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
    ...
    static async resolveStockAlert(id) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('stock_alerts')
                .update({
    ...
    static async getInventoryCounts(filters = {}) {
        try {
            let query = supabaseClient_1.supabaseAdmin
    ...
    static async createInventoryCount(countData) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('inventory_counts')
                .insert([countData])
    ...
    static async getInventoryCountItems(countId) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
    ...
    static async createInventoryCountItem(itemData) {
        try {
            const { data, error } = await supabaseClient_1.supabaseAdmin
                .from('inventory_count_items')
                .insert([itemData])
    ...
    static async getInventoryReport(filters = {}) {
        ...
        `);
            ...
            if (error)
                throw error;
            const totalValue = data?.reduce((sum, product) => sum + (Number(product.stock_quantity) * Number(product.unit_price)), 0) || 0;
            return {
                products: data || [],
                totalValue,
                filters
...
exports.InventoryClerkService = InventoryClerkService;
